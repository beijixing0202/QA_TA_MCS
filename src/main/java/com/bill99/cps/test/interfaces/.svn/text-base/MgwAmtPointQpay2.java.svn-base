...
public class MgwAmtPointQpay2 extends BaseTestCase{
	...
	// 平安积分支付
	@Test(dataProvider = "MgwAmtPointQpay2",description="平安积分支付二次快捷",timeOut=100000)
	
	public void amtPointQpay2(Map<String, String> datadriven){
	...
	// 实际结果与Reporter预期结果比较
		...
		// 数据库检查，如果预期结果与实际结果一致 并且是正常流程，数据库检查
				if (a1 && datadriven.get("comment").equalsIgnoreCase("平安积分支付二次快捷正常流程")) {
		...
		@DataProvider(name = "MgwAmtPointQpay2")
		public Iterator<Object[]> data1test() throws IOException {
			return ExcelProviderByEnv(this, "MgwAmtPointQpay2");
		}
