 ...
 * Author: Henry.jia
 */
public class MCS_ManualClearingTest extends BaseTestCase {
    @Autowired
    ...
    @Autowired
    private Pay pay;
    @Test(description = "MCS_手动过清分", dataProvider = "getData")
    public void ManualClearingTest(Map<String, String> data) throws InterruptedException {
        if (!data.isEmpty()) {
        ...
        Thread.sleep(5000);
        Reporter.log("----------调用MCS手动过清分接口----------");
        manualClearing.postingSuccess(responseData.get("idTxn"));
